...
import { LayoutDashboard, FileText, Send, Bell, LogOut, Shield, Menu, X, ChevronRight } from 'lucide-react'
import { useState } from 'react'

const navItems = [
  { label: 'Dashboard', path: '/bidder', icon: <LayoutDashboard size={20} /> },
  { label: 'Browse Tenders', path: '/bidder/tenders', icon: <FileText size={20} /> },
  { label: 'My Bids', path: '/bidder/bids', icon: <Send size={20} /> },
  { label: 'Notifications', path: '/bidder/notifications', icon: <Bell size={20} /> },
]
        ...
        <div className="flex items-center gap-3 px-4 h-16 border-b border-navy-800/50">
          ...
          {sidebarOpen && (
            <div className="animate-fade-in">
              ...
              <p className="text-[10px] text-navy-400 uppercase tracking-widest">Bidder Portal</p>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 border-b border-navy-800/50 bg-navy-900/50 backdrop-blur-xl flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <button className="relative text-navy-400 hover:text-navy-200 cursor-pointer"><Bell size={20} /><span className="absolute -top-1 -right-1 w-2 h-2 bg-accent-500 rounded-full" /></button>
